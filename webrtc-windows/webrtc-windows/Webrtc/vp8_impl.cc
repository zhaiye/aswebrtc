...
namespace webrtc
{
	...
	uint32_t VP8EncoderImpl::MaxIntraTarget(uint32_t optimalBuffersize)
	{
		...
		float scalePar = 0.5;
		uint32_t targetPct = (uint32_t)(optimalBuffersize * scalePar * codec_.maxFramerate / 10);
							   ...
							   const std::vector<FrameType>* frame_types)
	{
				 ...
				 codec_specific_info->codecType == kVideoCodecVP8)
		{
			if (feedback_mode_)
			{
				...
				{
					rps_.ReceivedRPSI((int)codec_specific_info->codecSpecific.VP8.pictureIdRPSI);
				}
			...
			else
			{
				if (codec_specific_info->codecSpecific.VP8.hasReceivedRPSI)
				{
					...
					int RPSI = (int)codec_specific_info->codecSpecific.VP8.pictureIdRPSI;
					for (size_t i = 0; i < encoders_.size(); ++i)
					{
