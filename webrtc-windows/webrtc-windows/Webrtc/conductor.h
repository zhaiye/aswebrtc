...
public:
	...
	uint8_t * VideoCapturerI420Buffer()
	{
		if (capturer && !bStop)
		{
			uint8_t* DataY = (uint8_t*)capturer->video_buffer->DataY();
			return DataY;
		}
		return nullptr;
	}
	...
	bool DataChannelSendText(const std::string & text);
	bool DataChannelSendData(const webrtc::DataBuffer & data);
...
public:
	BOOL bStop;
	BOOL bInitOK;
	fcbglobalCallback *fcbglobalcall;
	char socketId[256];
