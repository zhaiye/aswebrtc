...
Conductor::Conductor(int w, int  h)
{
	bStop = FALSE;
	bInitOK = FALSE;
...
void Conductor::AddStreams()
{
	...
	else
	{
		...
		vc = capturer = new YuvFramesCapturer2(*this);
	}
...
void Conductor::OnAddStream(rtc::scoped_refptr<webrtc::MediaStreamInterface> stream)
{
	...
	if (onRenderRemote)
	{
		...
		if (!tracks.empty())
		{
			...
			remote_video.reset(new VideoRenderer(*this, true, track));
		}
	}
	...
	if (audioEnabled)
	{
		...
		if (!atracks.empty())
		{
			...
			remote_audio.reset(new AudioRenderer(*this, true, track));
		}
	}
}
...
void Conductor::OnIceCandidate(const webrtc::IceCandidateInterface* candidate)
{
	...
	if (fcbglobalcall != NULL)
	{
		cJSON* root = cJSON_CreateObject();
		cJSON_AddStringToObject(root, "sdp_mid", candidate->sdp_mid().c_str());
		cJSON_AddNumberToObject(root, "sdp_mline_index", candidate->sdp_mline_index());
		cJSON_AddStringToObject(root, "sdp", sdp.c_str());
		cJSON_AddStringToObject(root, "socketId", socketId);

		char* out = cJSON_Print(root);

		cJSON_Delete(root);
		cjosnPackSend(FROM_SDK_WEBRTC, ZY_CLIENT_WEBRTC_ONICECANDIDATE, fcbglobalcall, out);
		free(out);
	}
}
...
void Conductor::OnSuccess(webrtc::SessionDescriptionInterface* desc)
{
	...
	{
		cJSON* root = cJSON_CreateObject();
		cJSON_AddStringToObject(root, "type", desc->type().c_str());
		cJSON_AddStringToObject(root, "sdp", sdp.c_str());
		cJSON_AddStringToObject(root, "socketId", socketId);
		char* out = cJSON_Print(root);
		cJSON_Delete(root);

		cjosnPackSend(FROM_SDK_WEBRTC, ZY_CLIENT_WEBRTC_ONSUCCESS, fcbglobalcall, out);
...
{
	if (bStop || !data_channel) return FALSE;
	return data_channel->Send(webrtc::DataBuffer(text));
}

bool Conductor::DataChannelSendData(const webrtc::DataBuffer & data)
{
	if (bStop || !data_channel) return FALSE;
	return data_channel->Send(data);
}
...
void Conductor::OnMessage(const webrtc::DataBuffer& buffer)
{
	...
	else
	{
		if (fcbglobalcall != NULL)
		{
			...
			char* out = cJSON_Print(root);
			cJSON_Delete(root);
			cjosnPackSend(FROM_SDK_WEBRTC, ZY_CLIENT_WEBRTC_ONMESSAGE, fcbglobalcall, out);
			free(out);
		}
	}
}
...
void Conductor::Run() {
	...
	BOOL ok = InitializePeerConnection();
	AddStreams();
	if (ok)
	{
		bInitOK = TRUE;
		while (1)
		{
			if (bStop || (data_channel != nullptr && data_channel->state() == webrtc::DataChannelInterface::DataState::kClosed))
			{
				bStop = TRUE;
				break;
			}
	...
	char* out = cJSON_Print(root);
	cJSON_Delete(root);
	cjosnPackSend(FROM_SDK_WEBRTC, ZY_CLIENT_WEBRTC_ONDATACHANNEL_CLOSE, fcbglobalcall, out);
	free(out);
}